...
"[project]/app/api/layouts/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET() {
    ...
    const layouts = await db.collection("screen_layouts").find({
    ...
    return Response.json(layouts.map((l)=>({
            _id: l._id.toString(),
            name: l.name,
            template: l.template
        })));
...
async function POST(req) {
    ...
    if (!template || !name || !Array.isArray(zone_data)) {
        return Response.json({
            error: "template, name, and zone_data are required"
        }, {
            status: 400
        });
    }
...
//# sourceMappingURL=%5Broot-of-the-server%5D__044956f1._.js.map